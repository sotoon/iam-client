...
func main() {
	...
	// Unbind role from user with items
	err = client.UnbindRoleFromUser(&workspaceUUID, &roleUUID, &userUUID, items)
	if err != nil {
		fmt.Println("error unbinding role from user:", err)
		os.Exit(1)
	}

	fmt.Println("Successfully unbound role from the user!")
}
