...
func main() {
	...
	// Bind role to service user with items
	err = client.BindRoleToServiceUser(&workspaceUUID, &roleUUID, &serviceUserUUID, items)
	if err != nil {
		fmt.Println("error binding role to service user:", err)
		os.Exit(1)
	}

	fmt.Println("Successfully bound role to the service user!")
}
