...
func main() {
	...
	err = client.UnbindServiceUserFromGroup(&workspaceUUID, &groupUUID, &serviceUserUUID)
	if err != nil {
		fmt.Println("error unbinding service user from group:", err)
		os.Exit(1)
	}

	fmt.Printf("Successfully unbound service user %s from group %s\n", serviceUserId, groupId)
}
