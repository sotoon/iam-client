...
func ensureStatusOK(resp *http.Response, successCode int) error {
	...
	var bepaErr types.BepaResponseError
	if err = json.Unmarshal(body, &bepaErr); err == nil {
		return createHTTPResponseError(resp.StatusCode, errors.New(bepaErr.Message.Detail))
	}
